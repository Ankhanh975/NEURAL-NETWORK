...
# def generate(action):
#     x = random.randrange(1, MAX_LEN)
#     y = random.randrange(1, MAX_LEN)
#     if action == 'add':
#         result = x+y
#         symbol = "+"
#     elif action == 'sub':
...
while len(equations) < hm_samples:
    nums = [random.randrange(1, max_val) for _ in range(
        random.randrange(2, max_number_of_nums))]

    number_of_parenthesis = random.randrange(
        0, min(4, len(nums)-2)) if len(nums) > 2 else 0
    ...
    for _ in range(number_of_parenthesis):
        ...
        closing_parenthesis_position = random.randrange(
            opening_parenthesis_position + 1, len(nums))
    ...
    for index, num in enumerate(nums):
        ...
        init_str += "{}{}{}{}".format(
            operator, '('*opening_parenthesis[index], str(num), ')'*closing_parenthesis[index])
